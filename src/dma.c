...
void DMA_ReadSync(u32 rom_addr, void* vram_addr, u32 length) {
    ...
    osPiStartDma(&iomesg, OS_MESG_PRI_NORMAL, OS_READ, rom_addr, vram_addr, length, &gDmaMessageQueue);
...
void DMA_Read(u32 device_addr, void* vram_addr, u32 length) {
    ...
    osInvalDCache(vram_addr, length);
    osPiStartDma(&iomesg, OS_MESG_PRI_NORMAL, OS_READ, device_addr, vram_addr, length, &gDmaMessageQueue);
}
