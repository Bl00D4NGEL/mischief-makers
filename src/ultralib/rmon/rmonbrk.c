...
static void ClearTempBreakpoint(void) {
    ...
    if (breakpoints[TMP_BP].breakAddress != NULL) {
        ...
        if ((inst & MIPS_BREAK_MASK) == MIPS_BREAK_OPCODE) {
            ...
            // If the target still holds a break instruction, restore the original word
...
int __rmonClearBreak(KKHeader* req) {
    ...
    // Clear the breakpoint and restore whatever was there before
    if (req->method == RMON_RSP) {
...
int __rmonSetSingleStep(int thread, u32* instptr) {
    ...
    if ((branchTarget & 3) != 0) {
        ...
        // Branch target is this instruction, so single-step cannot proceed here
        return FALSE;
    }
...
void __rmonHitSpBreak(void) {
    ...
    // Rewind RSP PC by one instruction to return to the break instruction
