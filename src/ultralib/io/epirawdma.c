...
s32 osEPiRawStartDma(OSPiHandle* pi_handle, s32 direction, u32 device_address, void* dram_address, u32 size) {
    u32 status;
    u32 domain;
    ...
    EPI_SYNC(pi_handle, status, domain);
    IO_WRITE(PI_DRAM_ADDR_REG, osVirtualToPhysical(dram_address));
    IO_WRITE(PI_CART_ADDR_REG, K1_TO_PHYS(pi_handle->baseAddress | device_address));
