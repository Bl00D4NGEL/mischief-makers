...
s32 osPiStartDma(OSIoMesg* mb, s32 priority, s32 direction, u32 dev_addr, void* dram_addr, u32 size, OSMesgQueue* mq) {
    register s32 ret;
    ...
    if (dev_addr & 1) {
        __osError(ERR_OSPISTARTDMA_DEVADDR, 1, dev_addr);
        return -1;
    }

    if ((u32)dram_addr & 7) {
        __osError(ERR_OSPISTARTDMA_ADDR, 1, dram_addr);
        return -1;
    }
